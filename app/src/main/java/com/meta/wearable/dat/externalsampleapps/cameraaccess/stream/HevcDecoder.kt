...
class HevcDecoder {
  ...
  fun stop() {
    ...
    try {
      ...
      Log.w(TAG, "Decoder stop during teardown: ${e.message}")
    }
  ...
  private fun onInputBuffer(codec: MediaCodec, index: Int) {
    ...
    try {
      ...
      if (shuttingDown || !active) {
        Log.d(TAG, "Ignoring input buffer callback during decoder teardown: ${e.message}")
      } else {
  ...
  private fun onOutputBuffer(codec: MediaCodec, index: Int, info: MediaCodec.BufferInfo) {
    try {
      ...
      if (shuttingDown || !active) {
        Log.d(TAG, "Ignoring output buffer callback during decoder teardown: ${e.message}")
      } else {
